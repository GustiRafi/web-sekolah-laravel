<div class="sidebar sidebar-dark sidebar-fixed" id="sidebar">
    <div class="sidebar-brand d-none d-md-flex">
        <h3>SMK N 1 BANTUL</h3>
    </div>
    <ul class="sidebar-nav" data-coreui="navigation" data-simplebar="">
        <li class="nav-item">
            <a class="nav-link active" href="/dashboard">Dashboard</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/berita">Berita</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/sambutan">Sambutan Kepala sekolah</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="">Guru</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="">siswa</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/jurusan">Jurusan</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/perpus">Perpustakaan</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/visimisi">Visi-Misi</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/sejarah">Sejarah</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/galery">Galery</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="/dashboard/testimoni">Testimoni</a>
        </li>
    </ul>
    <button class="sidebar-toggler" type="button" data-coreui-toggle="unfoldable"></button>
</div>